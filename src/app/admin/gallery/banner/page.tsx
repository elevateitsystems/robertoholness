...
export default function AdminGalleryBannerPage() {
  ...
  const [bannerData, setBannerData] = useState<any>({
    ...
    description: "Browse photos of our clean self-service dog wash stations, raw and organic pet food shelves, toys, and happy furry clients."
  });
  ...
  useEffect(() => {
    const saved = localStorage.getItem("banner_gallery");
    if (saved) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    localStorage.setItem("banner_gallery", JSON.stringify(updated));
    setBannerData(updated);
    
    setTimeout(() => {
      setSaving(false);
      toast.success("Gallery Banner Hero updated successfully!");
  ...
  return (
    <div className="space-y-8">
      <PageHeader 
        title="Manage Gallery Banner" 
        description="Click the edit icons on the preview below to update the Gallery Page hero headings, badges, and descriptions."
      />
      ...
      <div className="bg-gray-100 p-8 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">PageHero Component (Gallery Page)</span>
        </div>
        ...
        <div className="w-full bg-gradient-to-br from-[#FFFDF9] via-[#FFF3EB] to-[#FFFDF9] py-16 px-6 rounded-xl border border-[#F2E5DC] text-center relative overflow-hidden shadow-md">
          ...
          >
            ...
            <div className="flex items-center justify-center gap-2 mb-4 text-xs text-secondary/50 font-medium">
              <span>Home</span>
              <span>&gt;</span>
              <span className="text-secondary font-semibold">Gallery</span>
            </div>
      ...
      <Modal
        ...
        title="Update Gallery Banner Hero"
